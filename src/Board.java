...
public class Board extends JPanel {
	...
	public Board(String whiteOrder, String blackOrder) {
		...
		dice = new DButton[4];
		playerAtMove = 0;
		setupBoard(whiteOrder,blackOrder, true);
	}
	...
	/**
	 * Gets the number of players of a specific player in a specific point.
	 * @param player
	 * @param pointnumber
	 * @return
	 */
	...
	public void setActivePlayer(int player) {
		...
		if(player==1)
		{
			...
			updateGameLines(pm.getPossibleSelections(), false);
			if(pm.cantMove())
			{
		...
		{
			setPlayerClicked(false); // if the player made it quicker than the Java Application, disable to prevent future errors.
			lines[currentSelection].setSelected(false);
			timerMode = "RollDice";
	 ...
	 * line to move to.
	 */
	public void movePlayer(int from, int to, int time, boolean last_move) {
		Timer t = new Timer(time*1000, new MyActionListenerDelayTimer(from, to, last_move));
		t.start();
	}
	 ...
	 */
	public void updateGameLines(boolean active[], boolean choice) {
		for (int i = 0; i < 28; i++)
		{
			lines[i].setActive(active[i]);
			if(choice && i != getCurrentSelection()) 
				lines[i].setBorderPainted(active[i]);
			else if(!choice)
				lines[i].setBorderPainted(false);
		}
	}
	 ...
	 */
	private void setupBoard(String whiteOrder, String blackOrder, boolean fromSave) {
		currentSelection = 1;
		playerClicked = false;
		...
		if(whiteOrder.equals("0"))
			whiteOrder = setupOrder;
		if(blackOrder.equals("0"))
			blackOrder = setupOrder;
		System.out.printf("White string: %s, black string: %s\n", whiteOrder, blackOrder);
		for (int i = 0; i < 28; i++)
			checkers1[i] = Character.getNumericValue(whiteOrder.charAt(i));
		for (int i = 25, j = 0; i >= 0; i--, j++)
			checkers2[i] = Character.getNumericValue(blackOrder.charAt(j));
		for(int i = 26; i < 28; i++)
			checkers2[i] = Character.getNumericValue(blackOrder.charAt(i));
		if(fromSave) // swap checkers2
		{
			for(int i = 0; i < 26/2; i++)
			{
				checkers2[i] += checkers2[25-i];
				checkers2[25-i] = checkers2[i] - checkers2[25-i];
				checkers2[i] -= checkers2[25-i];
			}
		}
		for (int i = 0; i < 28; i++)
		...
		for (int i = 0; i < 28; i++) {
			...
			lines[i].setBorder(BorderFactory.createLineBorder(Color.blue));
			if (i < 13)
	...
	public class MyActionListenerDelayTimer implements ActionListener {
		int from = 0, to = 0;
		boolean change_turn = false;
		public MyActionListenerDelayTimer(int f, int t, boolean last_move) {
			from = f;
			to = t;
			change_turn = last_move;
		}
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			else if(timerMode=="InitiateAI")
			{
				...
				for (int i = 0; i < m.getCount(); i++)
					movePlayer(m.getFrom(i), m.getTo(i), i, i+1 == m.getCount());
				animationTimer.stop();
			}
		}
	}
	...
	public class MyActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			{
				int[] whiteArray = getBoardAsArray(1), blackArray = getBoardAsArray(2);
				File f = new File("gammon.save");
				if(f.exists())
					f.delete();
				try {
					FileWriter fw = new FileWriter(f);
					System.out.printf("Saving white array:");
					for(int i = 0; i < whiteArray.length; i++)
					{
						fw.write(Integer.toString(whiteArray[i]));
						System.out.printf("%d", whiteArray[i]);
					}
					System.out.printf("Saving black array:");
					for(int i = 0; i < blackArray.length; i++) {
						fw.write(Integer.toString(blackArray[i]));
						System.out.printf("%d", blackArray[i]);
					}
					fw.flush();
					fw.close();
					...
					if (!hasPlayerClicked()) {
						...
						if (temp.getLineID() == currentSelection) {
							...
							if (diceInfo.finished() || pm.cantMove())
								setActivePlayer(2);
							else // else because otherwise setActivePlayer also calls winnerSequence
								winnerSequence(); 
						}
					}
				}
